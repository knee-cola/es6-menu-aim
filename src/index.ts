...
class MenuAim {
  ...
  attach(menu: HTMLElement): void {
    ...
    // Добавляем event listeners для элементов меню
    const rows = this.menu.querySelectorAll(this.options.rowSelector)

    rows?.forEach((row: HTMLElement) => {
      row.addEventListener('mouseenter', this.mouseenterRow)
      row.addEventListener('mouseleave', this.mouseleaveRow)
      row.addEventListener('click', this.clickRow)
    })
  ...
  activate(row: HTMLElement): void {
    ...
    const newRow = this.options.activate(row)
  ...
  activationDelay(): number {
    ...
    if (this.activeRow) {
      ...
      const rows: NodeListOf<HTMLElement> = this.activeRow.parentElement.querySelectorAll(this.options.submenuSelector)
    ...
    const offset = _offset(this.menu)
    const upperLeft = {
      x: offset.left,
    ...
    const loc = _mouseLocs[_mouseLocs.length - 1]
    let prevLoc = _mouseLocs[0]
    ...
    const decreasingSlope = _calcSlope(loc, decreasingCorner)
    const increasingSlope = _calcSlope(loc, increasingCorner)
    const prevDecreasingSlope = _calcSlope(prevLoc, decreasingCorner)
    const prevIncreasingSlope = _calcSlope(prevLoc, increasingCorner)
 ...
 * @private
 */
function _mousemoveDocument(ev: MouseEvent): void {
 ...
 * @private
 */
function _calcSlope(a: Coordinates, b: Coordinates): number {
 ...
 * @private
 */
function _offset(elem: HTMLElement): ElementOffset {
  const rect: ClientRect = elem.getBoundingClientRect()
  const win: Window = elem.ownerDocument.defaultView
